...
The plugin needs Advanced Custom Fields to work properly. If the plugin is installed without Advanced Custom Fields, a warning on the Wordpress admin dashboard will pop up, advising that Advanced Custom Fields is installed. This GF Books SK Plugin will work without ACF, but will not display all the extra fields for the book custom post type.
...
1. Upload `gf-books-sk-plugin.php` to the `/wp-content/plugins/` directory
