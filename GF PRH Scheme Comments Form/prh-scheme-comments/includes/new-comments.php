...
<div class="fl-comments">
	...
	comment_form( array(
		...
		'title_reply'           => _x( 'Say hello', 'Respond form title.', 'fl-automator' ),
		...
		'comment_field'         => '<label for="comment">' . _x( 'What would you like to share?', 'Comment form label: comment content.', 'fl-automator' ) . '</label>
        ...
        'fields'                => apply_filters( 'prh_scheme_comment_form_default_fields', array(

			'author'                 => '<label for="author">' . _x( 'Name', 'Comment form label: comment author name.', 'fl-automator' ) . ( $req ? __( ' (so you can get to know each other)', 'fl-automator' ) : '' ) . '</label>
										<input type="text" name="author" class="form-control" value="' . $comment_author . '" tabindex="1"' . ( $req ? ' aria-required="true"' : '' ) . ' /><br />',

			'email'                  => '<label for="email">' . _x( 'Email', 'Comment form label: comment author email.', 'fl-automator' ) . ( $req ? __( ' (you’ll need to put this in, but it won’t be visible)', 'fl-automator' ) : '' ) . '</label>
